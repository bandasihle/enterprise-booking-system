...
<body>
    ...
    <section class="content-grid">
      <div class="panel">
        <div class="panel-header">
          ...
          <button class="panel-btn" onclick="window.location.href='bookings.jsp'">View All</button>
        </div>
        <div class="table-wrapper">
          <table>
            <thead>
              <tr>
                <th>Student</th><th>Lab</th><th>Seat</th><th>Date</th><th>Status</th>
              </tr>
            </thead>
      ...
      <div class="panel">
        ...
        <div class="quick-actions">
          ...
          <button class="action-btn" onclick="window.location.href='resources.jsp'">Manage Labs</button>
          <button class="action-btn" onclick="window.location.href='complaints.jsp'">View Complaints</button>
  ...
  (function () {
    ...
    /* ---- STAT CARDS + DYNAMIC SUB-NOTES ---- */
    function loadStats() {
      fetch('../api/admin/dashboard')
        .then(function(r) { return r.json(); })
        .then(function(data) {
          document.getElementById('totalUsers').textContent      = data.totalUsers      != null ? data.totalUsers      : '—';
          document.getElementById('totalBookings').textContent   = data.totalBookings   != null ? data.totalBookings   : '—';
          document.getElementById('availableLabs').textContent   = data.availableLabs   != null ? data.availableLabs   : '—';
          document.getElementById('totalComplaints').textContent = data.totalComplaints != null ? data.totalComplaints : '—';

          var activeUsers   = data.activeUsers       != null ? data.activeUsers       : 0;
          var todayBookings = data.todaysBookings    != null ? data.todaysBookings    : 0;
          var pendingCmpl   = data.pendingComplaints != null ? data.pendingComplaints : 0;
          var availLabs     = data.availableLabs     != null ? data.availableLabs     : 0;

          document.getElementById('noteUsers').textContent      = activeUsers   + ' active';
          document.getElementById('noteBookings').textContent   = todayBookings + ' today';
          document.getElementById('noteLabs').textContent       = availLabs     + ' operational';
          document.getElementById('noteComplaints').textContent = pendingCmpl   + ' pending';
        })
        .catch(function() {
      ...
      Promise.all(promises)
        .then(function(results) {
          ...
          if (includeBookings && bookingsData) {
            ...
            doc.text('Total bookings: ' + bookings.length, 14, y); y += 5;
            if (!bookings.length) {
              ...
              doc.autoTable({
                ...
                body: bookings.map(function(b) {
                  return [b.student_name||b.student_email||'—', b.lab_name||'—',
                          b.seat_label||b.seat_id||'—', b.booking_date||'—', b.status||'—'];
                }),
                theme: 'striped',
          ...
          if (includeUsers && usersData) {
            ...
            doc.text('Total: ' + users.length + '   Active: ' + activeCount + '   Banned: ' + bannedCount, 14, y); y += 5;
            if (!users.length) {
              doc.setTextColor(150,150,150); doc.text('No users found.', 14, y);
            } else {
              doc.autoTable({
                startY: y,
                ...
                body: users.map(function(u) {
                  return [u.full_name||(u.email?u.email.split('@')[0]:'—'),
                          u.email||'—', u.role||'—', u.is_banned?'Banned':'Active'];
                }),
                theme: 'striped',
                headStyles: { fillColor:[37,99,235], textColor:255, fontStyle:'bold', fontSize:9 },
                bodyStyles: { fontSize:8.5 },
